...
import { messages, users, voiceFeedback } from "@db/schema"; // Added import for voiceFeedback
...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/messages/:messageId/text-to-speech", async (req, res) => {
    ...
    try {
      ...
      const { audioBuffer, voiceSettings } = await voiceService.convertTextToSpeech(
        message.content,
      ...
      res.set({
        ...
        "X-Voice-Settings": JSON.stringify(voiceSettings)
      });
